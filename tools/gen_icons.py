...
#
# 架線と水平線は 4132 自身の図式実測による。**長さではなく「中心からの距離」で取る。**
# 図式の架線は円の中心線から生えるが、こちらは円の外縁から生やすので、長さで写すと
# その差ぶん先端が外へ出る。基準は円の中心線の直径（図式5.955pt / アイコンD4132-W）。
#   架線1 先端 = 中心から 1.325×中心線外径・212.8°（画面座標・Y下向き・右が0°）
#   架線2 先端 = 中心から 1.174×中心線外径・341.3°
#   水平線 半長 = 0.419×中心線外径
# **架線2本は長さが違う。** 41-19 は 0.914/0.841 と近かったので単一値に丸めたが、
# ここは開きが大きく、丸めると片方が必ずずれる（#18）。
# もとは 1.0×外径・210.1°/344.3° を 41-42 電力柱と共有していた。電力柱は実測
# 0.906/1.014/1.015 で 1.0 が正しく、電話柱にだけ誤っていた。定数は共有しない。
s = 9.3
D4132 = 1.0 * s                              # 円の外径
DC4132 = D4132 - W                           # 円の中心線の直径（図式と比べる基準）
TIP_A = 1.325 * DC4132                       # 架線1の先端（中心から）
TIP_B = 1.174 * DC4132                       # 架線2の先端（中心から）
CHORD = 0.419 * DC4132                       # 水平線の半長
ARM = 1.0 * s                                # 41-42 電力柱の架線の長さ
...
d = (ring(CX, CY, DC4132 / 2) +
     seg((CX - CHORD, CY), (CX + CHORD, CY)) +
     haisen(D4132, TIP_A - D4132 / 2, 212.8) +
     haisen(D4132, TIP_B - D4132 / 2, 341.3))
write('4132', d, f'円 外径{D4132:.1f}px＋水平線＋架線2本'
      f'(先端{TIP_A:.1f}px/212.8°・{TIP_B:.1f}px/341.3°)')

# ---- 41-42 電力柱（点E5＋方向E6） ------------------------------------
# 図式(p.77): 円φ1.0＋架線3本。実測で 30.3°/173.3°/323.9°。
# 架線の長さは実測 0.906/1.014/1.015×外径で、3本とも 1.0×外径でよい。
# 41-32 電話柱は 0.878/0.657 で違うので、ARM は共有しない（#18）。
...
#   架線 = 円の縁から 0.861×外径（実測 0.914/0.841 が近いので単一値に丸めた。
#          41-32 電話柱は 0.878/0.657 と開くので2本を別長で描く。兄弟でも寸法は違う）
...
# ---- 34-01 門 / 52-26 滝（大きさの直し・#15） -------------------------
# どちらも形は図式の「極小」と合っていたが、**2件だけ 6.25px/mm で描かれていた**。
# 小物体系の実寸スケールは 9.3px/mm（マンホール極小φ2.0mm→18.56px）なので、
# 家の標準の 2/3 の大きさだった。図式の mm 寸法から描き直す。
#
# 図式の図は**ページごとに縮尺が違う**（門は 2.75pt/1.0mm、滝は 10.80pt/1.5mm）
# ので、描画コマンドの pt ではなく寸法欄の mm を使う。
MM = 9.3        # px/mm。小物体系の実寸スケール

# 34-01 門（p64）: 極小は 1.0mm の四角。左に並ぶ真形（門柱の外周＝壁の切れ目）は
# 記号ではないので採らない。9.3px は基準の下限10pxをわずかに割るが、同じ 1.0mm の
# 41-19 有線柱（円9.3px）と揃えるほうを採る。
D3401 = 1.0 * MM
h = (D3401 - W) / 2
write('3401', poly_band([(CX - h, CY - h), (CX + h, CY - h),
                         (CX + h, CY + h), (CX - h, CY + h)]),
      f'四角 外形{D3401:.2f}px（図式の極小1.0mm）')

# 52-26 滝（p96）: 極小は 1.5mm の横線＋下に小円2つ。**既存アイコンは小円が
# 図式より下に離れていた**（図式の間隔/幅は0.38だが0.54で描かれていた）。真形と
# 極小が同じ塊として基準形状に入っていたため、照合では出ていなかった。
# 図式の極小の座標（横線は中心線10.80pt、小円は左右で径がわずかに違い非対称に置かれる）
# から起こす。**図式の寸法は中心線なので、インクは線幅0.45ptぶん外へ出る。** 比べる
# のはインクなので、横線11.25pt・小円2.68/2.72pt として倍率を取る（13.95/11.25=1.24）。
K5226 = 1.5 * MM / (10.80 + 0.45)                 # インク基準の倍率 1.24
d = seg((CX - (10.80 + 0.45) / 2 * K5226, CY - 2.5575 * K5226),
        (CX + (10.80 + 0.45) / 2 * K5226, CY - 2.5575 * K5226))
for dx, dia in ((-3.315, 2.68), (3.71, 2.715)):   # 図式の中心x（欄中心から）とインク径
    d += ring(CX + dx * K5226, CY + 2.5575 * K5226, (dia * K5226 - W) / 2)
write('5226', center_ink(d), f'横線{1.5 * MM:.2f}px＋小円2つ（図式の極小1.5mm）')
...
# ======================================================================
# 豊中市の拡張DMコード（道路台帳平面図・図郭57-08）
#
# 標準図式に定義が無いコードなので、意匠と寸法は市が公開しているサンプルの
# ベクタPDF図面（DM-_57-08.pdf、1:500）から起こした。記号は1つの塗りパスで
# 描かれているので、代表点にいちばん近いパスを取り出して外形と内部の線を測る。
# 既知の 4191（0.773m）・4151（1.058m）で検算して、#22 の実測と一致している。
#
# 大きさは図面の実寸ではなく既存の同系統アイコンに揃える（ガイド「3. 大きさを
# 決める」。図面の実寸に合わせるのは利用側の icon-size の仕事で、スプライト側で
# 小さく描くと潰れる ← 22-35 雨水桝の教訓）。
# ======================================================================

D_EXT = 18.62      # 41xx の円。既存のマンホール系7つと同径
HW = 0.9           # ハッチの線幅。1.1px だと図面の間隔（外径比0.079）が潰れる


def hatched_circle(dia, ratios, hw=HW):
    """円＋内部の水平線。ratios は外径に対する y（下が正）。線は円の内側で止める。"""
    r = dia / 2
    d = ring(CX, CY, r - W / 2)
    r_in = r - W
    for t in ratios:
        y = CY + t * dia
        half = math.sqrt(max(r_in ** 2 - (y - CY) ** 2, 0.0)) - hw / 2
        if half > 0.4:
            d += seg((CX - half, y), (CX + half, y), w=hw)
    return d


# ---- 41-33 / 41-34 / 41-44 / 41-45（電話系・電気系の空き番） ----------
# いずれも図面では外形0.540mの円で、中に水平のハッチが入る。本数と入る範囲が
# 4コードの唯一の違い（図面のハッチ位置を外径比で測った値をそのまま使う）。
write('toyonaka-4133', hatched_circle(
    D_EXT, [-0.3825, -0.245, -0.1275, 0.0095, 0.1275, 0.245, 0.3625]),
    '円18.62px＋全面に横線7本（図面 0.540m・間隔は外径比0.118）')
write('toyonaka-4134', hatched_circle(
    D_EXT, [-0.3435, -0.2255, -0.1075, 0.0095]),
    '円18.62px＋上半分に横線4本（図面 0.540m・間隔は外径比0.118）')
write('toyonaka-4144', hatched_circle(
    D_EXT, [0.0095, 0.1275, 0.2255, 0.3435]),
    '円18.62px＋下半分に横線4本（図面 0.540m・間隔は外径比0.118）')
write('toyonaka-4145', hatched_circle(
    D_EXT, [0.0095, 0.1075, 0.1865, 0.2845, 0.3625]),
    '円18.62px＋下半分に横線5本（図面 0.540m・間隔は外径比0.079）')

# ---- 追加しなかったコード（既存アイコンで代替できる）------------------
# 図面から起こした案を tools/verify_shapes.py --similar にかけた結果、次の4コード
# （4147 は 4146 と幾何が完全に同じなので5コード）は既存アイコンと同じ形だった。
# 同じ絵を別のキーで二重に配らないので、このリポジトリには追加しない。
# 利用側が既存のキーを指し、その対応と根拠は利用側のプロジェクトに記録する。
#
#   4143  素の円（図面 0.540m）        → dm-3519  比率差0.0% 覆い率100/100 同形
#   2224  塗りの点（図面 0.254m）      → dm-8199  比率差0.0% 覆い率100/100 同形
#   4181  四角の枠（図面 1.30〜1.43m） → dm-2235  比率差0.0% 覆い率100/89.6 近い（目視で同一）
#   4146  円＋中心の点（図面 0.413m）  → dm-7303  比率差0.0% 覆い率97.0/89.3 近い（目視で同一）
#   4147  4146 と同一の幾何            → dm-7303

# ---- 22-48 -----------------------------------------------------------
# 図面では外形1.048mの円に「案」。22-42 道路標識案内の系統と読める。
write('toyonaka-2248',
      ring(CX, CY, (D_EXT - W) / 2) + glyph('案', 12.1, CX, CY),
      f'円{D_EXT:.2f}px＋「案」12.1px（図面 1.048m / Noto Sans JP 400）')

# ---- 22-25 -----------------------------------------------------------
# 図面では外形0.413×0.254mの塗りつぶした長円（両端が半円のスタジアム形）。
W2225, H2225 = 10.0, 6.16
r2225 = H2225 / 2
dx2225 = W2225 / 2 - r2225
d = (dot(CX - dx2225, CY, r2225) + dot(CX + dx2225, CY, r2225) +
     poly([(CX - dx2225, CY - r2225), (CX + dx2225, CY - r2225),
           (CX + dx2225, CY + r2225), (CX - dx2225, CY + r2225)]))
write('toyonaka-2225', d,
      f'塗りつぶしの長円 {W2225:.1f}×{H2225:.2f}px（図面 0.413×0.254m）')

# ---- 42-14 -----------------------------------------------------------
# 図面では外形0.921×0.466mの三角形の旗＋右へ伸びる柄。旗の中に縞が3本入る。
# 図面の頂点（外形比）A(-0.5,+0.218) B(+0.121,+0.253) C(-0.098,-0.253)、
# 柄は BC の中ほどから (+0.49,-0.21) へ。縞は BC と平行なので、A から BC へ
# 向かう比 t で引く（図面は不等間隔だが、この大きさでは等間隔に丸める）。
K4214 = 18.0
A = (CX - 0.5 * K4214, CY + 0.218 * K4214)
B = (CX + 0.121 * K4214, CY + 0.253 * K4214)
C4214 = (CX - 0.098 * K4214, CY - 0.253 * K4214)
d = poly_band([A, B, C4214])
d += seg(((B[0] + C4214[0]) / 2, (B[1] + C4214[1]) / 2),
         (CX + 0.49 * K4214, CY - 0.21 * K4214))
for t in (0.3, 0.55, 0.8):
    p = (A[0] + (B[0] - A[0]) * t, A[1] + (B[1] - A[1]) * t)
    q = (A[0] + (C4214[0] - A[0]) * t, A[1] + (C4214[1] - A[1]) * t)
    d += seg(p, q, w=HW)
write('toyonaka-4214', center_ink(d),
      f'三角形の旗（縞3本）＋柄 幅{K4214:.1f}px（図面 0.921×0.466m）')


# =====================================================================
# 交通施設（22-xx）の未作成分。図式PDF p.53〜58。
#
# 大きさは既存の22xxアイコンから校正する。円の記号は 22-61 電話ボックス・
# 22-62 郵便ポスト（どちらも図式φ2.5mm＝外径22px）に合わせ、**px/mm = 8.8** を
# 基本にする。角丸箱の 22-47 だけは同じ記号を持つ 22-46 信号灯（s=7.9）に揃える。
# 形の比率は図式の描画コマンドの実測（pt）どおりにする。
# =====================================================================

KOTSU = 8.8           # px/mm。22-61/22-62（図式2.5mm＝22px）からの校正値


# ---- 22-41 道路情報板（方向E6） --------------------------------------
# 図式(p.53): 幅1.5・全高2.0・板の高さ0.5。実測(pt)は 9.52×11.45 で、板の下端は
# 上から 4.91（＝全高の0.429）。上辺・左右の縦線・板の下辺の4本で、脚が下に伸びる。
h2241 = 2.0 * KOTSU
w2241 = h2241 * (9.52 / 11.45)
top = CY - h2241 / 2 + W / 2
bot = CY + h2241 / 2 - W / 2
xl, xr = CX - w2241 / 2 + W / 2, CX + w2241 / 2 - W / 2
yb = top + (4.91 / 11.45) * h2241                 # 板の下辺
d = (seg((xl, top), (xr, top)) + seg((xl, yb), (xr, yb)) +
     seg((xl, top), (xl, bot)) + seg((xr, top), (xr, bot)))
write('2241', center_ink(d),
      f'板{w2241:.1f}×{yb - top:.1f}＋脚2本・全高{h2241:.1f}px')

# ---- 22-47 信号灯専用ポールのないもの（方向E6） -----------------------
# 図式(p.53): 22-46 信号灯と同じ角丸箱2.0×1.0（灯φ0.3を3つ）で、挿入点の小円と
# 連結線が無いもの。**大きさは 22-46 の箱と同じにする**（同じ記号なので揃える）。
s = 7.9
bw, bh = 2.0 * s, 1.0 * s
bx0 = CX - bw / 2
d = round_rect_band(bx0 + W / 2, CY - bh / 2 + W / 2,
                    bx0 + bw - W / 2, CY + bh / 2 - W / 2, bh / 3 - W / 2)
for i in (1, 2, 3):
    d += dot(bx0 + bw * i / 4, CY, 0.3 * s / 2)
write('2247', d, f'角丸箱{bw:.1f}×{bh:.1f}px・灯3つ（22-46からポールを外したもの）')

# ---- 22-51 交通量観測所（点E5） --------------------------------------
# 図式(p.53): 円φ2.5に水平の直径線。円は 22-61/22-62 と同径。
D = 2.5 * KOTSU
d = ring(CX, CY, (D - W) / 2) + seg((CX - D / 2 + W, CY), (CX + D / 2 - W, CY))
write('2251', d, f'円 外径{D:.1f}px＋水平の直径線')

# ---- 22-52 スノーポール（点E5） --------------------------------------
# 図式(p.53): 円φ2.0の中に「S」。字は塗りなので描画コマンドには出ず、図式の図から採る。
# 字高は円の内側に収まる 0.66×外径（3531 保の 0.735 は漢字なので、ラテン字は少し下げる）。
D = 2.0 * KOTSU
d = ring(CX, CY, (D - W) / 2) + glyph('S', 0.66 * D, CX, CY)
write('2252', d, f'円 外径{D:.1f}px＋「S」{0.66 * D:.1f}px（Noto Sans JP 400 / OFL）')

# ---- 22-55 距離標（km）（点E5） --------------------------------------
# 図式(p.54): 四角い枠の中に△。実測(pt)は枠 11.18×8.83、△は底辺6.33・高さ4.2。
# 幅を2.0mm＝17.6pxとし、比率は実測どおり。図式では△の2辺が頂点で閉じていないが、
# この大きさでは差が出ないので閉じた三角形にする。
w2255 = 2.0 * KOTSU
h2255 = w2255 * (8.83 / 11.18)
x0, y0 = CX - w2255 / 2, CY - h2255 / 2
k = w2255 / 11.18
tri_w, tri_h = 6.33 * k, 4.2 * k
base_y = y0 + (6.36 / 8.83) * h2255
d = (rect_band(x0 + W / 2, y0 + W / 2, x0 + w2255 - W / 2, y0 + h2255 - W / 2) +
     poly_band([(CX - tri_w / 2, base_y), (CX, base_y - tri_h),
                (CX + tri_w / 2, base_y)]))
write('2255', center_ink(d),
      f'枠{w2255:.1f}×{h2255:.1f}px＋△（底辺{tri_w:.1f}・高さ{tri_h:.1f}px）')

# ---- 22-56 距離標（m）（点E5） ---------------------------------------
# 図式(p.54): 22-55 と同じ枠に○。実測(pt)は枠 11.17×11.81、円φ6.62。
# 22-55 と枠の幅をそろえる（同系統の兄弟なので）。
w2256 = 2.0 * KOTSU
h2256 = w2256 * (11.81 / 11.17)
dia2256 = w2256 * (6.62 / 11.17)
x0, y0 = CX - w2256 / 2, CY - h2256 / 2
d = (rect_band(x0 + W / 2, y0 + W / 2, x0 + w2256 - W / 2, y0 + h2256 - W / 2) +
     ring(CX, CY, (dia2256 - W) / 2))
write('2256', center_ink(d),
      f'枠{w2256:.1f}×{h2256:.1f}px＋円φ{dia2256:.1f}px')

# ---- 22-63 火災報知器（点E5） ----------------------------------------
# 図式(p.54): 円φ2.0の中に、上へ伸びる縦線と下へ開く2本（逆Y字）、左右に小円φ0.3。
# 実測(pt)は円 10.36×10.66、分岐点は円心のやや下 (4.97,5.74)、小円の中心 (2.69,3.49)
# と (7.67,3.49)、小円φ1.24。比率は実測どおりに写す。
D = 2.0 * KOTSU
k = D / 10.66
org = (CX - 10.36 / 2 * k, CY - 10.66 / 2 * k)      # 図式bboxの左上をキャンバスに置く


def P(x, y):
    return (org[0] + x * k, org[1] + y * k)


d = (ring(CX, CY, (D - W) / 2) +
     seg(P(4.97, 1.23), P(4.97, 5.74)) +
     seg(P(4.97, 5.74), P(2.07, 8.61)) +
     seg(P(4.97, 5.74), P(7.46, 8.61)) +
     dot(*P(2.69, 3.49), 1.24 * k / 2) +
     dot(*P(7.67, 3.49), 1.24 * k / 2))
write('2263', center_ink(d),
      f'円 外径{D:.1f}px＋逆Y字＋小円φ{1.24 * k:.1f}px 2つ')

# ---- 24-19 鉄道のトンネル（方向E6） ----------------------------------
# 図式(p.58): 極小は幅1.5の半円のアーチ（∩）。脚は無い（アーチの下にある矩形は
# 寸法欄の枠で、記号のインクではない）。22-19 道路のトンネルが1/3円なのに対し、
# こちらは半円。**図式の線幅が0.9pt（他の倍）**なので、42-55 波浪観測所と同じ扱いで
# 線幅を1.8pxにする（基準の1.1pxだと実表示で消える）。
W2419 = 1.8
w2419 = 1.5 * KOTSU
r2419 = w2419 / 2 - W2419 / 2
