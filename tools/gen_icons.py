...
# ---- 35-31 保健所 ----------------------------------------------------
# 図式: 円の中に「保」。直径 4.0mm / 2.5mm。円は3519と同径、字は内接する高さに。
write('3531', ring(CX, CY, 11 - W / 2) + glyph('保', 12.5, CX, CY, weight=400),
      '円22px＋「保」高さ12.5px（Noto Sans JP 400 / OFL）')

# ---- 63-35 はい松地 --------------------------------------------------
# 図式: 下向きの矢羽根（∨）＋中央の縦棒。2.0×1.5mm(500-2500) / 1.5×1.2mm(5000)。
# 植生系の既存（6334 荒地 13.5×12.5）に合わせ、幅14px・高さ10.5px（=2.0:1.5）とする。
w6335, h6335 = 14.0, 10.5
top = CY - h6335 / 2
bot = CY + h6335 / 2
d = (polyline([(CX - w6335 / 2, top), (CX, bot), (CX + w6335 / 2, top)]) +
     seg((CX, top), (CX, bot)))
write('6335', d, '∨＋中央縦棒 14×10.5px')
