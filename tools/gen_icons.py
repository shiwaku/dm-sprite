...
# ---- 41-32 電話柱（点E5＋方向E6） ------------------------------------
# 図式(p.76): 円φ1.0の中に水平線。図式に描かれている斜めの線は柱から出る架線
# （＝方向E6が表す向き）の図示なので、点記号には含めない。
# 隣の41-19 有線柱は「円＋垂直線」、41-42 電力柱は「円のみ」で区別される。
# φ1.0mmは9.3px相当だが、既存の最小級（7302 水準点10.94・4238 防犯灯11.94）に
# 合わせて外径11pxとする。
D = 11.0
d = (ring(CX, CY, (D - W) / 2) +
     seg((CX - (D / 2 - W), CY), (CX + (D / 2 - W), CY)))
write('4132', d, f'円 外径{D:.0f}px＋水平線')

# ---- 41-42 電力柱（点E5＋方向E6） ------------------------------------
# 図式(p.77): 円φ1.0のみ（斜めの線は架線の図示）。4132と同径にする。
write('4142', ring(CX, CY, (11.0 - W) / 2), '円 外径11px')
