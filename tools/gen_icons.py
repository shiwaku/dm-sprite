...
# ---- 41-19 有線柱（点E5＋方向E6） ------------------------------------
# 図式(p.76): 円＋縦線＋架線2本。実測で架線は 213.8°/341.2°（画面座標）。
# 41-32 電話柱（円＋水平線）・41-42 電力柱（円のみ）の兄弟なので、円と架線の
# 寸法はそちらに合わせる。取得位置は柱の中心なので円の中心を (32,32) に置く。
d = (ring(CX, CY, (D4132 - W) / 2) +
     seg((CX, CY - (D4132 / 2 - W)), (CX, CY + (D4132 / 2 - W))) +
     haisen(D4132, ARM, 213.8) + haisen(D4132, ARM, 341.2))
write('4119', d, f'円 外径{D4132:.1f}px＋縦線＋架線2本(213.8°/341.2°)')
