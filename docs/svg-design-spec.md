...
- ただし方向レイヤ（E6）で取得位置と記号の重心がずれる記号は、**取得位置**を (32, 32) に置く（41-19 有線柱・41-32 電話柱・41-42 電力柱は円の中心。回転軸が画像中心なので、外すと記号が公転する）

## 形の検証

図式と一致しているかは `tools/verify_shapes.py` が判定し、`data/shape-baseline.csv` に1件ずつ記録される。CI（`.github/workflows/shape-check.yml`）でも走るので、形を変えると基準値の差分がレビューに出る。手順は[アイコン作成ガイド](icon-authoring-guide.md)を参照。
